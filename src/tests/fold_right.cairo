...
mod tests {
    ...
    fn success_fold_1_right() {
        ...
        dispatcher.fold(Direction::Right(()), 1);
        
        let mut grid = dispatcher.get_paper();

        assert_eq(grid.at(0), @('D'.into()) , 'Wrong cell 0 init value');
        assert_eq(grid.at(1), @('C'.into()) , 'Wrong cell 1 init value');
        assert_eq(grid.at(2), @('BA'.into()) , 'Wrong cell 2 init value');
        assert_eq(grid.at(3), @('H'.into()) , 'Wrong cell 3 init value');
        assert_eq(grid.at(4), @('G'.into()) , 'Wrong cell 4 init value');
        assert_eq(grid.at(5), @('FE'.into()) , 'Wrong cell 5 init value');
        assert_eq(grid.at(6), @('L'.into()) , 'Wrong cell 6 init value');
        assert_eq(grid.at(7), @('K'.into()) , 'Wrong cell 7 init value');
        assert_eq(grid.at(8), @('JI'.into()) , 'Wrong cell 8 init value');     
    }
}